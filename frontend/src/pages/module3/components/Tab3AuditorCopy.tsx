/**
 * Tab 3 — Auditor's Copy (NAS Repository + Upload per langkah)
 *
 * Layout:
...
export default function Tab3AuditorCopy({ programId, folderName }: {
  programId: string;
              ...
              <ul className="divide-y divide-slate-100">
                {evidenceList.map((ev) => (
                  <li key={ev.id} className="px-3 py-2 flex items-center gap-3">
                    ...
                    <button
                      onClick={() => {
                        if (confirm(`Hapus "${ev.nama_asli}"?`)) deleteEv.mutate(ev.id);
                      }}
